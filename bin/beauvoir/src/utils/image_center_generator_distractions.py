...
from .blender_utils import rotate_object2
...
from .blender_utils import get_center_coordinates


class ImageCenterGenerator():
    def __init__(self, data, save_path, distract,
                 ...
                 distract_num=0,
                 back_distract=False,
                 back_path='/home/hippoc/bin/beauvoir/data/Back_Distractions/'):
        ...
        self.data = glob.glob(data + '*.obj')
        self.blender_save_path = '../data/cache/current_scene.blend'
        self.save_path = save_path
        self.distract = glob.glob(distract + '*.obj')
        ...
        self.distract_range = distract_range
        self.image_count = 0
        self.back_distract = True
        self.back_path = glob.glob(back_path + '*.obj')
    ...
    def make_image_name(self, class_name, arg, box_coordinates):
        ...
        image_name = self.save_path + str(self.image_count)
        self.image_count += 1
        return image_name
    ...
    def construct_scene(self, filepath, class_name):
        ...
        if self.back_distract == True:
            back = load_obj(random.choice(self.back_path))
            rotation = uniform(*[0, 360], size=3)
            rotate_object(back, rotation.tolist())

            #options = [[5, 0, 0], [0, 5, 0]]
            options = [[-5.5, 0, 0], [-5.5, 5.5, 0]]
            translate_object(back, random.choice(options))

            change_color(back)

        for _ in range(randint(self.distract_num)):
            distr = load_obj(random.choice(self.distract))

            rotation = uniform(*[0, 360], size=3)
            rotate_object(distr, rotation.tolist())

            translation = uniform(*self.distract_range, size=2)
            translation += np.array([2, 0])
#            idx = np.random.choice(3, 2, replace=False)
#            translation[idx] *= -1
            translate_object(distr, [-2] + translation.tolist())
        ...
        if self.rotation_range is not None:
            base = np.array([0.0, 70.0, -45.0])
            rotation = uniform(*self.rotation_range, size=3) + base
        ...
        if self.background == 'plain':
            ...
            add_random_patch_background(image, box_size=200)
